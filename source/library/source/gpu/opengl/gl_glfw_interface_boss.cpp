...
namespace hive
{
    namespace interface
    {
        ...
        InterfaceBoss::InterfaceBoss(unsigned w, unsigned h) : Boss(IDENTIFIER)
        {
            width  = w;
            height = h;
        }
        ...
        void InterfaceBoss::teardown() {}


        hive::interface::keyboard::Keyboard::KeyStateReader & InterfaceBoss::getKeyboard()
        {
            return glfwKeyboard.getKeyStateReader();
        }


        hive::interface::controller::Controller::ControllerStateReader &
        InterfaceBoss::getController(unsigned index)
        {
            return controllers[index % MAX_AVAILABLE_CONTROLLERS].getControllerStateReader();
        }


        hive::interface::mouse::Mouse::MouseStateReader & InterfaceBoss::getMouse()
        {
...
//############################## END DEBUG
        ...
        bool InterfaceBoss::resize(unsigned int w, unsigned int h,
                                   hive::interface::SCREEN_MODE mode)
        {
            return true;
        };
    } // namespace interface
