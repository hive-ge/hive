...
public: //Member Functions
	...
	friend std::ostream &operator<<(std::ostream &os, const tvec2<T> &vec)
	{
	...
	friend tvec2<T> operator*(T s, tvec2<T> &vr)
	{
		return tvec2<T>(vr.x * s, vr.y * s);
	}

	friend tvec2<T> operator*(tvec2<T> &vl, T s)
	{
		return s * vl;
	}
	...
	inline tvec2<T> negate()
	{
		return tvec2<T>(-x, -y);
	}

	inline tvec2<T> round()
	{
		return tvec2<T>(std::round(x), std::round(y));
	}

	inline T dot(tvec2<T> &v)
	{
		return x * v.x + y * v.y;
	}
