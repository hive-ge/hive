...
public:
	...
	tQuadraticBezier2D(T x1, T y1, T x2, T y2, T x3, T y3) : p1(x1, y1), p2(x2, y2), p3(x3, y3){};
	tQuadraticBezier2D(tvec2<T> _p1, tvec2<T> _p2, tvec2<T> _p3) : p1(_p1), p2(_p2), p3(_p3){};

	inline tQuadraticBezier2D<T> reverse() { return tQuadraticBezier2D<T>(p3.x, p3.y, p2.x, p2.y, p1.x, p1.y); }
	...
	inline void split(T t, tQuadraticBezier2D<T> &left = tQuadraticBezier2D<T>(), tQuadraticBezier2D<T> &right = tQuadraticBezier2D<T>())
	{
		left.p1 = p1;
		right.p3 = p3;
	...
	{
		return (rotate(-atan2(line.p2.y - line.p1.y, line.p2.x - line.p1.x), line.p1) - line.p1) / line.length();
	}
	...
	inline unsigned char lineIntersections(tline2D<T> &line, T tn = T(0.0), T tp = T(0.0))
	{
		...
		T
			y1 = rc.p1.y,
			y2 = rc.p2.y, y3 = rc.p3.y, x1 = rc.p1.x, x2 = rc.p2.x, x3 = rc.p3.x,
			a = y1 - T(2) * y2 + y3,
			b = T(2) * (y2 - y1),
			c = y1;
	...
	inline bool lineSegments(tline2D<T> line, tQuadraticBezier2D<T> *curve_array, unsigned &curve_length, tline2D<T> *line_array, unsigned &line_length)
	{
		line_length = 0;
		curve_length = 0;
		...
		T y1 = rc.p1.y, y2 = rc.p2.y, y3 = rc.p3.y, x1 = rc.p1.x, x2 = rc.p2.x, x3 = rc.p3.x;
		...
		if (HAVE_RESULT)
			line_array[line_length++] = last_new_line;
